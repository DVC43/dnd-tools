(function () {
  ...
  function buildAideDDUrl(name) {
	...
	function rgUpdateFilterAvailability() {
	  const creatureSel = document.getElementById("rgCreatureSelect");
	  const alignSel = document.getElementById("rg-align-filter");
	  const legendarySel = document.getElementById("rg-legendary");
	  if (!creatureSel || !alignSel || !legendarySel) return;

	  if (creatureSel.value !== "any") {
		// Specific creature chosen → alignment and legendary filters no longer matter
		alignSel.disabled = true;
		legendarySel.disabled = true;
	  } else {
		// Back to "Any" creature → allow filters again
		alignSel.disabled = false;
		legendarySel.disabled = false;
	  }
	}
  ...
  function rollReagent() {
    ...
    const legendaryOnly = document.getElementById("rg-legendary").value === "true";
    ...
    const pool = RG_SOURCES.filter(src => {
      if (creatureFilter === "any" && legendaryOnly && !src.legendary) return false;
	  if (creatureFilter !== "any" && src.name !== creatureFilter) return false;
      if (creatureFilter === "any" && !matchesAlignment(src.alignment, alignFilter)) return false;
    ...
    if (!pool.length) {
      errorEl.textContent = creatureFilter === "any"
        ? "No creatures match the current CR / alignment / legendary filters."
        : "No creatures match the selected creature / CR / legendary filters.";
      ...
      document.getElementById("rg-aidedd-link").href = "#";
      document.getElementById("rg-aidedd-link").setAttribute("aria-disabled", "true");
	...
	document.getElementById("rg-aidedd-link").setAttribute("aria-disabled", "false");
